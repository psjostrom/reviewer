...
If test files were modified, check: are they testing behavior or implementation details? Do they match the testing philosophy in CLAUDE.md? Would they break on a refactor that doesn't change behavior?
...
- **NOT YOUR SCOPE:** bugs in production code — that's the Bug Hunter. Code style in tests — only flag if CLAUDE.md has test-specific rules. Loading/error state handling — that's the **Error & Edge Cases** agent (don't flag "tests don't test error states" — flag "no test exists for this critical code path").
