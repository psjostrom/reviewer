...
1. Read all CLAUDE.md files in the repo (root + any in directories touched by the changes).
2. Get the diff:
...
| Agent | subagent_type |
|-------|---------------|
| 1 — Bug Hunter | `reviewer:bug-hunter` |
| 2 — Guidelines Checker | `reviewer:guidelines` |
| 3 — Error & Edge Cases | `reviewer:error-edges` |
| 4 — Architecture & Quality | `reviewer:architecture` |
| 5 — Test Reviewer | `reviewer:test-reviewer` |

### Domain Agents (based on detected project)

| Project | Agent | subagent_type |
|---------|-------|---------------|
| Strimma | S1 — Coroutine & Lifecycle | `reviewer:strimma-coroutine` |
| Strimma | S2 — Medical Data Integrity | `reviewer:strimma-medical` |
...
- **11-25**: Nitpick. Stylistic preference not backed by CLAUDE.md or clear bug evidence.
...
- **76-90**: Important. Verified real issue. Will impact functionality or violates explicit CLAUDE.md rule.
...
- **CLAUDE.md violations** — Open the CLAUDE.md and verify the rule is actually stated, not paraphrased from memory.
...
2. For each selected issue, read the file and apply the fix using Edit.
